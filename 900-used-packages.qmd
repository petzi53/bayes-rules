...
- **Chapter 3**: 
    ...
    - `plot_beta_binomial()`: Given information on the prior (`alpha` and `beta`) and data (`y` and `n`), this function produces a plot of any combination of the corresponding prior pdf, scaled likelihood function, and posterior pdf. All three are included by default.
    - `summarize_beta_binomial()`: Given information on the prior (`alpha` and `beta`) and data (`y` and `n`), this function summarizes the mean, mode, and variance of the prior and posterior Beta models of $\pi$.
- **Chapter 5**:
    - `plot_gamma()`: Plots the probability density function (pdf) for a $\text{Gamma}(shape, rate)$ model of variable $\lambda$.
    - `plot_poisson_likelihood()`: Plots the Poisson likelihood function for variable $\lambda$ given a vector of Poisson counts `y`.
    -`plot_gamma_poisson()`: Given information on the prior (`shape` and `rate`) and data (the sample size `n` and `sum_y`), this function produces a plot of any combination of the corresponding prior pdf, scaled likelihood function, and posterior pdf. All three are included by default.
    - `summarize_gamma_poisson()`: Given information on the prior (`shape` and `rate`) and data (the sample size `n` and `sum_y`), this function summarizes the mean, mode, and variance of the prior and posterior Gamma models of $\lambda$.
    - `plot_normal()`: Plots the probability density function (pdf) for a Normal(mean, $sd^2$) model of variable 
    - `plot_normal_likelihood()`: Plots the Normal likelihood function for variable 
$\mu$ given a vector of Normal data $y$.
    - `plot_normal_normal()`: Given information on the prior (mean and sd) and data (the sample size n, mean y_bar, and standard deviation sigma), this function produces a plot of any combination of the corresponding prior pdf, scaled likelihood function, and posterior pdf.
    - `summarize_normal_normal()`: Given information on the prior (`mean` and `sd`) and data (the sample size `n`, mean `y_bar`, and standard deviation `sigma`), this function summarizes the mean, mode, and variance of the prior and posterior Normal models of $\mu$.
...
For instance: `patchwork:::"-.ggplot"(p1, p2)` or more detailed and included the graphs directly:

```{r}
#| label: fig-900-patchwork-side-by-side
#| fig-cap: Using patchwork without `library()` command for figures side-by-side
#| fig-height: 3
#| code-fold: false

patchwork:::"-.ggplot"(
  ggplot2::ggplot(iris) +
      ggplot2::geom_point(ggplot2::aes(Petal.Length, Petal.Width)),
  ggplot2::ggplot(iris) +
      ggplot2::geom_point(ggplot2::aes(Sepal.Length, Sepal.Width))
)
```
