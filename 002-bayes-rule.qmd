...
## Building a Bayesian model for numerical random variables
...
chess_long <- chess_posterior |>
  select(pi, prior, likelihood, posterior) |>
  pivot_longer(cols = c(prior, likelihood, posterior), 
               names_to = "model", 
               values_to = "probability") |>
  mutate(model = factor(model, levels = c("prior", "likelihood", "posterior")))

# Create three separate lollipop graphs side by side with same y-scale
ggplot(chess_long, ggplot2::aes(x = factor(pi), y = probability, color = model)) +
  geom_point(size = 4, shape = 21, fill = "white") +
  geom_segment(ggplot2::aes(xend = factor(pi), yend = 0), color = "black", alpha = 0.7) +
  geom_text(ggplot2::aes(label = scales::percent(probability, 1)), 
...
::::{.my-procedure-container}
 ...
 1. Construct a `r glossary("prior_model", "prior model")` for your variable of interest, $\pi$. The prior model specifies two important pieces of information: 
    (a) the possible values of $\pi$ and 
    (b) the relative prior plausibility of each.  
2. Summarize the dependence of data $Y$ on $\pi$ via a `r glossary("CPMF", "conditional pmf")` $f (y \mid \pi)$.  
3. Upon observing data $Y = y$, define the `r glossary("Likelihood-Function", "likelihood function")` $L(\pi \mid y) = f (y \mid \pi)$ which encodes the relative likelihood of observing data $Y = y$ under different $\pi$ values.  
4. Build the `r glossary("posterior_model", "posterior model")` of $\pi$ via Bayes’ Rule which balances the prior and likelihood:
